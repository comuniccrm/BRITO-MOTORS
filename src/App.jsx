...
function App() {
  ...
  const getSafeMapUrl = () => {
    ...
    if (mapped.includes('/place/')) {
      try {
        const placeChunk = mapped.split('/place/')[1].split('/')[0];
        const placeName = decodeURIComponent(placeChunk.replace(/\+/g, ' '));
    ...
    return `https://maps.google.com/maps?q=${encodeURIComponent(mapped)}&t=&z=15&ie=UTF8&iwloc=&output=embed`;
  };
